...
object Record {
  val p0 = DateTimeFormatter.ofPattern("E MMM d HH:mm:ss VV yyyy")
  ...
  def dateParse(d: String): ZonedDateTime = {
    ...
    val r0 = Try(ZonedDateTime.parse(t, p0)).toOption
    ...
    r0.orElse(r1).orElse(r2).orElse(r3).getOrElse(throw new IllegalArgumentException(s"Cannot parse date: '$t'"))
  }

  def validate(r: Record): Try[Record] = Try {
    r.ws.foreach { w =>
      if (w.length > 60 || w.contains(" "))
    ...
    if (r.commands.isEmpty) {
      throw new IllegalArgumentException(s"Unexpected command (none): $r")
    }
    //if (r.commands.exists(_.matches("^[0-9]+.*")))
    //  throw new IllegalArgumentException(s"Unexpected command (number there): $r")
    r
  }
...
def main(iFile: String, oFile: String) = {
  val echoDate3SessionDashWorspaceBranchCommands = "echo (\\w{3} \\w{3} \\d+ [0-9:]+ \\w+ \\d{4}) (\\w+) - ws: (.*?) branch: (.*?);(.*)".r
  ...
  val echoDate3SessionDashWorspaceCommands = "echo (\\w{3} \\w{3} \\d+ [0-9:]+ \\w+ \\d{4}) (\\w+) - ws: (.*?);(.*)".r
  ...
  val records = Source.fromFile(iFile).getLines.filterNot(_.isEmpty).map(s => s.replaceAll("  ", " ")).map { original =>
    original match {
      case echoDate3SessionDashWorspaceBranchCommands(date, session, ws, branch, cmd) => Record.from(0, or = original, date = Some(date), session = Some(session), ws = Some(ws), branch = Some(branch), commands = Some(cmd))
      ...
      case echoDate3SessionDashWorspaceCommands(date, session, ws, cmd) => Record.from(12, or = original, date = Some(date), session = Some(session), ws = Some(ws), branch = None, commands = Some(cmd))

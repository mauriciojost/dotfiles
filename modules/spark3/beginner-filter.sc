...
spark.sparkContext.setJobDescription("Filter airports and write output")

val airports = spark.read.format("avro").load("datasets/optd_por_public_all.avro2/")
val asianAirports = airports.where(airports("continent_name") === "Asia")
asianAirports.write.parquet(output)
